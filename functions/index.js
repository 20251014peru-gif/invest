...
async function assertAuthorized(uid) {
  ...
  if (!snap.exists || snap.data()?.enabled !== true) {
    throw new HttpsError('permission-denied', 'This Firebase UID is not allowed to spend AI budget.');
  }
}
...
function outputConfig() {
  return {format: {type: 'json_schema', schema: ANALYSIS_OUTPUT_SCHEMA}};
}
...
async function reserveBudget(uid, reserveUsd, cfg) {
  ...
  await db.runTransaction(async tx => {
    const [ds, ms] = await Promise.all([tx.get(dref), tx.get(mref)]);
    const d = ds.exists ? ds.data() : {};
    const m = ms.exists ? ms.data() : {};
...
async function reconcileBudget(keys, reserveUsd, actualUsd, unreconciled = false) {
  ...
  await db.runTransaction(async tx => {
    for (const ref of [dref, mref]) {
      const snap = await tx.get(ref); const v = snap.exists ? snap.data() : {};
      const next = {
...
function priceSnapshot(model, price, pricing) {
  return {
    model,
    inputPricePerMillion: Number(price.inputPerMillion),
    outputPricePerMillion: Number(price.outputPerMillion),
    cacheReadPerMillion: Number(price.cacheReadPerMillion || 0),
    cache5mWritePerMillion: Number(price.cache5mWritePerMillion || 0),
...
export const estimateEventAnalysis = onCall({region: REGION, secrets: [ANTHROPIC_API_KEY], timeoutSeconds: 60, memory: '256MiB'}, async request => {
  ...
  const policy = choosePolicy(request.data?.tier);
  const price = getPrice(pricing, policy.model);
  const prompt = buildAnalysisPrompt(request.data?.event, request.data?.thesis);
  ...
  const maxCostUsd = estimateCostUsd(inputTokens, policy.maxTokens, price);
  const cfg = await settings();
  return {
    provider: 'anthropic', tier: policy.tier, model: policy.model,
    estimatedInputTokens: inputTokens,
    estimatedOutputTokens: policy.estimateOutputTokens,
    ...
    note: 'Estimate only. Final cost uses API response usage.'
  };
});
...
export const analyzeEvent = onCall({region: REGION, secrets: [ANTHROPIC_API_KEY], timeoutSeconds: 180, memory: '512MiB'}, async request => {
  ...
  const aref = db.collection('event_ai_analysis').doc(fingerprint);
  const existing = await aref.get();
  if (existing.exists && existing.data()?.status === 'success') {
    return {cacheHit: true, apiCalled: false, incrementalCostUsd: 0, analysis: existing.data()};
  }
  ...
  const budgetKeys = await reserveBudget(uid, reserveMaxCostUsd, cfg);
  const usageRef = db.collection('ai_usage').doc();
  const snapshot = priceSnapshot(policy.model, price, pricing);
  ...
  try {
    ...
    const actualUsd = costFromUsageUsd(message.usage || {}, price);
    await reconcileBudget(budgetKeys, reserveMaxCostUsd, actualUsd, false);
    const completedAt = new Date().toISOString();
    const analysisDoc = {
      schema: 'event-ai-analysis/1', status: 'success', reviewRequired: true,
      g6Unlocked: false, eventId: prompt.event.event_id, fingerprint,
      provider: 'anthropic', tier: policy.tier, model: policy.model,
      promptVersion: PROMPT_VERSION, output,
      usage: message.usage || {}, estimatedCostUsd,
      costFromUsageUsd: actualUsd, billedCostUsd: null,
      priceSnapshot: snapshot, pricingStale: pricingIsStale(pricing),
      requestedAt, completedAt, uid,
      createdAt: FieldValue.serverTimestamp()
    };
    await aref.set(analysisDoc, {merge: false});
    await usageRef.set({
      schema: 'ai_usage/1', status: 'success', eventId: prompt.event.event_id,
      analysisType: 'risk_thesis', fingerprint, uid, provider: 'anthropic',
      tier: policy.tier, model: policy.model, promptVersion: PROMPT_VERSION,
      ...
      estimatedInputTokens: inputTokens, estimatedCostUsd,
      reservedMaxCostUsd, costFromUsageUsd: actualUsd, billedCostUsd: null,
      priceSnapshot: snapshot, requestedAt, completedAt,
      createdAt: FieldValue.serverTimestamp()
    });
    return {cacheHit: false, apiCalled: true, incrementalCostUsd: actualUsd, analysis: analysisDoc};
  } catch (err) {
    const usage = err?.usage || null;
    const actualUsd = usage ? costFromUsageUsd(usage, price) : 0;
    await reconcileBudget(budgetKeys, reserveMaxCostUsd, actualUsd, !usage);
    const completedAt = new Date().toISOString();
    await usageRef.set({
      schema: 'ai_usage/1', status: 'error', eventId: prompt.event.event_id,
      analysisType: 'risk_thesis', fingerprint, uid, provider: 'anthropic', tier: policy.tier,
      model: policy.model, promptVersion: PROMPT_VERSION, usage: usage || null,
      estimatedInputTokens: inputTokens, estimatedCostUsd, reservedMaxCostUsd,
      costFromUsageUsd: usage ? actualUsd : null, billedCostUsd: null,
      costStatus: usage ? 'USAGE_RECONCILED' : 'UNRECONCILED',
      errorType: err?.name || 'Error', errorMessage: String(err?.message || err).slice(0, 500),
      priceSnapshot: snapshot, requestedAt, completedAt,
      createdAt: FieldValue.serverTimestamp()
    });
