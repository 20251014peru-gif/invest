...
(async()=>{
  ...
  try{
    ...
    for(const id of ['stockInput','fDate','fBody','fUserJudge','checkAddBtn'])assert.ok(await p.locator('#'+id).isVisible(),id+' visible without opening additional fields');
    ...
    for(const kind of ['idea','chart','news','youtube','memo']){await p.locator('#kindPick [data-k="'+kind+'"]').click();assert.equal(await p.locator('#fBody').inputValue(),'고객사의 재고 감소와 주문 회복이 동시에 나타나는지 확인한다.');}
    await p.screenshot({path:path.join(output,'02-record.png'),fullPage:true});
    await p.locator('#mSave').click();await p.waitForSelector('body:not(.rw-active)');assert.ok(await p.evaluate(()=>records.some(r=>r.title==='반도체 업황 점검'&&r.userJudgment==='다음 실적 발표 전까지 관찰한다.')));
    ...
    await p.locator('#stockAddFollowup').click();await p.waitForSelector('#followupModal.on');await p.locator('#fuQuestion').fill('다음 분기 영업이익률이 15%를 넘는가?');await p.locator('#fuExpected').fill('재고가 줄면서 이익률이 개선될 것으로 예상');await p.locator('#fuDue').fill('2026-10-25');await p.locator('#fuResult').fill('발표 자료에서 16%를 확인했다.');await p.locator('#fuJudgment').selectOption('keep');await p.locator('#fuNext').fill('다음 분기에도 유지되는지 확인');
    ...
    assert.deepEqual(errors,[]);console.log('PASS layout: visible input order, kind switching retains text, saving, 3 desktop columns, 3 viewport widths, 2/3-column PDFs, long record pagination fixture');
