...
- **붙여넣기 안내 (v7.28.2)**: 색상 매핑·제목 단계 조정·MarkFlow 표기 변환 같은 사소한 서식 변경은 안내하지 않는다. 사진·본문 일부가 누락되거나(임시·로컬 주소 이미지, 원본 접근 불가, 기기 저장 동영상 등) 링크 연결이 제거된(허용되지 않은 주소) 경우에만 도구줄 아래 고정된 한 자리(`#stPasteNotice`)에 접힌 요약("⚠ 붙여넣기 안내 N건")으로 표시된다. 붙여넣을 때마다 새로 쌓이지 않고 이 세션에서 모인 내용을 한 목록으로 갱신하며, 확인 버튼 없이 필요할 때만 펼쳐보면 된다. 누락 감지·위험 링크 제거(`safeHref`) 자체는 그대로 동작 — 바뀐 건 "무엇을 안내로 보여줄지"뿐이다.
...
- 기록보관실에는 휴지통이 없다(삭제 = 즉시 영구 삭제). 운영 검수용 시험 노트는 사용자 결정에 따라 당분간 유지(아래 §10).

## 10. 작업·검수 기록
| 버전 | 커밋 | 날짜(KST) | 내용 |
|---|---|---|---|
| v7.27 | `7974a83` | 2026-09-17 11:50 | 공부노트·서식 편집기·MarkFlow 가져오기·노트 파일 내보내기/복원 최초 반영 |
| v7.27.1 | `064098b` | 2026-09-17 13:30 | 운영 검수에서 발견한 복원 시 이미지 조용한 손실 수정, 비용 설명 보완 |
| **v7.27.2** | `119bcc6` | 2026-09-17 14:00 | 휴대폰 편집창 수정(하단 도구줄·더보기·선택 유지·보이는 영역 맞춤·표 가로 스크롤) — **모바일 검수 완료 버전** |
| v7.28.2 | `3312110` | 2026-09-17 19:00 | 붙여넣기 안내 정리 — 사소한 서식 변경은 침묵, 사진·본문 누락/링크 제거만 본문 밖 한 자리에 접힌 요약으로(쌓임·확인버튼 제거) |

### 10.1 검수 근거 구분
- **사용자 실제 기기 확인 (v7.27.2, 2026-09-17)**: 사용자가 실제 휴대폰에서 재시험해, 앞서 보고한 문제가 모두 해결됐다고 확인.
  - 보고됐던 문제: 편집창이 작게 깨짐, 기능 적용 시 창 깨짐, 길게 눌러 선택 후 색 변경 불가(휴대폰 메뉴만 표시), 도구가 많아 가로 스크롤, 가로 회전 시 배치 고정.
  - 같은 확인에서 사진 저장·노트 저장은 수정 전부터 정상.
  - 기기 기종·브라우저 정보는 기록되지 않음.
- **개발 측 확인 (실기기 아님)**: 데스크톱 브라우저의 화면 크기 흉내(360×740, 740×360, 375×812, 812×375)로 측정.
  - 도구줄 한 줄 7개·가로 넘침 없음, 선택이 풀린 상황의 색·굵게 적용, 넓은 표 폭 제한, 회전 후 창 높이·저장 버튼 표시.
  - 한글 입력·실제 터치 선택·실제 키보드 동작은 이 방식으로 검증하지 않았다(실기기 확인으로 대체).
- **운영(라이브) 확인 (v7.27~v7.27.1, 개발 측)**: 배포본과 main 파일 일치, 운영 저장·새로고침 재열기·재수정, Storage 이미지 업로드, 노트 파일에 실제 이미지 포함, 원본 이미지 접근 불가 가정 복원(파일 포함 시 복원됨 / 파일 미포함 시 누락 표시).
- **자동 시험**: 단위 14 + research 33 통과, 편집기 회귀 페이지 6/6(로컬·배포본), `Study notes checks` CI 성공.
  - `Research workspace checks` 의 `chatgpt/tests` 실패 2건은 `5af458a`(09-16)부터 이어진 기존 문제로, 이번 작업 범위 밖.

### 10.2 운영 시험 데이터 (사용자 결정: 삭제하지 않고 유지)
| 종류 | 식별 | 비고 |
|---|---|---|
| 공부노트 | `fov2fy66dQC2nt4Z3G6G` · “[운영검수·삭제예정] 공부노트 시험 0917” | 2판, 이전 판 1개 |
| 공부노트 | `qqGm4a7qsKwTVPl8pDx8` · “[운영검수·삭제예정] 복원 시험 A (원본 이미지 접근 불가 가정)” | 노트 파일 복원으로 생성 |
| 이미지 | `records_images/2026-09-17/study_img_mu50kyqv_98emza.png` | 2.9KB |
| 이미지 | `records_images/2026-09-17/study_img_mu50mo44_mfh55s.png` | 2.9KB(복원 시 재업로드) |

### 10.3 상태
- 이번 작업(공부노트·서식 편집기·MarkFlow 가져오기 + 운영·모바일 검수)은 **v7.27.2 로 완료**.
- 새 기능 수정 없음. 남은 과제는 §9 참고.
