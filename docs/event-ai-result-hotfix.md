...
## Korean output and disclosure evidence follow-up

`event-risk-v3-ko-source` requests Korean narrative, retains machine enum codes, and requires units/period/comparison basis for any figure from source text. The UI uses Korean labels and explicitly shows an unassessable thesis impact when thesis data is absent or evidence is LOW. No-data requests return a deterministic Korean report before token counting, reservation or any provider call; they never unlock G6. Existing English responses are not silently translated with a paid API. A new paid analysis requires the normal explicit cost confirmation.

The collector can retrieve official DART document.xml ZIPs for events without structured facts, with bounded byte/text sizes and request counts. Receipt identity, source URL, fetch status and truncation are preserved. Extracted body text remains distinct from calculated metrics; no automatic materiality upgrade is made. The manual Event Source Refresh workflow refreshes one existing receipt without notifications or AI. Some exchange disclosures may be unavailable through this API; those remain visibly unavailable and cannot trigger an empty paid analysis.
