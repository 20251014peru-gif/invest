...
export async function legacyItems(records,todos,needsReview=()=>false){
  ...
  for(const r of records){
    ...
    if(needsReview(r)||(r.kind==='study'&&r.reviewAt))out.push({id:await stableId('review:'+r.id+':'+(r.reviewAt||r.studyStatus||'')),question:r.title||'공부노트 재검토',dueAt:r.reviewAt||'',originalDueAt:r.reviewAt||'',state:'open',result:'',expectation:'공부노트 재검토',judgment:'pending',comparison:'',sourceIds:[r.id],sourceSnapshots:[{id:r.id,title:r.title||'',kind:r.kind||''}],topics:r.topics||[],stocks:r.stocks||[],followUpType:'reread',legacy:{key:'review:'+r.id+':'+(r.reviewAt||r.studyStatus||''),reviewAt:r.reviewAt||''}});
  }
...
export function filterItems(items,{state='undone',q='',comparison='',judgment='',today=day(Date.now())}={}){
  return items.filter(x=>state==='all'||state==='done'?state==='all'||x.state==='done':state==='paused'?x.state==='paused':x.state==='open'||x.state==='working')
    ...
    .filter(x=>!q||[x.question,x.result,x.expectation,x.changeReason,x.observedChange,x.lesson,(x.topics||[]).join(' '),(x.stocks||[]).join(' '),(x.links||[]).map(l=>l.url+' '+(l.title||'')).join(' '),(x.sourceSnapshots||[]).map(r=>r.title).join(' ')].join(' ').toLowerCase().includes(q.toLowerCase()))
