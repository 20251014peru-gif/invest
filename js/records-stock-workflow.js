...
function stockFocusHTML(name,summary){
  ...
  return '<div class="stockFocusHd"><b style="font-size:20px">'+esc(name)+'</b><span><button type="button" class="infoBtn" id="stockAddRecord">＋ 기록 남기기</button><button type="button" class="infoBtn" data-stockopen="'+esc(name)+'">종목 정보 수정</button><button type="button" class="infoBtn" id="stockPrint">출력 보기</button></span></div>'
    +'<div class="stockDashboard"><section class="stockOverview" aria-label="종목 요약"><div class="stockSectionHd"><h3>종목 요약</h3></div>'+(info.core.length?fields(info.core):'<p class="stockHint">보유 상태·수량·단가는 종목 정보에서 입력할 수 있습니다.</p>')
    ...
    +'</section><section class="stockDetails"><div class="stockSectionHd"><h3>판단 기준 · '+info.details.length+'</h3></div>'+(info.details.length?fields(info.details):'<p class="stockHint">종목 정보 수정에서 목표·위험 조건 등 필요한 기준을 추가하세요.</p>')+'</section>'
    +'<section class="stockChecks"><div class="stockSectionHd"><h3>다음 확인'+(FU&&FU.ready?' · '+summary.pending.length:'')+'</h3><button type="button" class="infoBtn" id="stockAddFollowup">＋ 확인할 질문</button></div>'+status
    +((done.length||paused.length)?'<details class="stockCompleted"><summary>완료·복기 '+done.length+'건'+(paused.length?' · 보류 '+paused.length+'건':'')+'</summary><p class="stockHint">완료한 확인의 결과와 판단 변화를 열어 보세요.</p>'+done.concat(paused).map(stockTaskHTML).join('')+'</details>':'')+'</section></div>'
...
function showStockPrint(name){
  ...
  panel.innerHTML='<div class="stockPrintActions"><button type="button" class="infoBtn" data-print-close>← 종목으로 돌아가기</button><label>본문 배치 <select id="stockPrintColumns"><option value="2">2열 · 넓게 읽기</option><option value="3">3열 · 모아 보기</option></select></label><span>A4 가로 · 요약 3열 / 본문 선택 열</span><button type="button" class="infoBtn" data-print-now>인쇄 / PDF 저장</button></div><h1>'+esc(name)+' · 투자 기록</h1><p class="printDate">저장된 내용 기준 · 출력 준비 '+esc(new Date().toLocaleString('ko-KR'))+'</p>'
    +'<section><h2>종목 요약·판단 기준</h2><dl class="stockPrintSummary">'+summary.fields.map(function(f){return '<div><dt>'+esc(f.label)+'</dt><dd>'+esc(f.value)+'</dd></div>';}).join('')+'</dl></section>'
    +'<div class="stockPrintColumns"><h2>확인·복기</h2>'+(!(FU&&FU.ready)||FU.error?'<p>확인 기록 연결이 완료되지 않아 이 부분을 출력할 수 없습니다.</p>':summary.followups.length?summary.followups.map(function(x){return '<article><h3>'+esc(x.question)+'</h3><p class="printMeta">상태: '+esc({open:'미확인',working:'확인 중',paused:'보류',done:'완료'}[x.state]||x.state)+' · 예정일: '+esc(x.dueAt||'미정')+(x.completedAt?' · 완료일: '+RecordStocks.kstDate(x.completedAt):'')+'</p>'+[['당시 예상',x.expectation],['확인 결과',x.result],['판단 변화',({pending:'아직 판단 안 함',keep:'유지',change:'수정',withdraw:'철회'})[x.judgment]],['예상 비교',({same:'예상과 같음',partial:'일부 다름',different:'예상과 다름',unknown:'판단 유보'})[x.comparison]],['변화 이유',x.changeReason],['다음 행동',x.nextAction],['이후 관찰',x.observedChange],['배운 점',x.lesson]].filter(function(f){return f[1];}).map(function(f){return '<p><b>'+f[0]+'</b><br>'+esc(f[1])+'</p>';}).join('')+(x.links||[]).map(function(l){return '<p>근거: '+esc(l.url)+'</p>';}).join('')+stockPrintImages(x.assets||[])+'</article>';}).join(''):'<p>저장된 확인 기록이 없습니다.</p>')+''
    +'<h2>기록·변경 이력</h2>'+summary.entries.map(function(r){return '<article><h3>'+esc(r.title||'기록')+'</h3><p class="printMeta">'+esc(r.date||'날짜 미기록')+'</p>'+(r.oneLiner&&r.body&&r.oneLiner!==r.body?'<p><b>핵심 한 줄</b><br>'+esc(r.oneLiner)+'</p>':'')+'<p>'+esc(r.body||r.summary||r.oneLiner||'본문은 원본 기록에서 확인하세요.')+'</p>'+(r.userJudgment?'<p><b>내 판단</b><br>'+esc(r.userJudgment)+'</p>':'')+(r.link?'<p>원본: '+esc(r.link)+'</p>':'')+stockPrintImages(r.attachments||(r.image?[{url:r.image}]:[]))+'</article>';}).join('')+'</div><p class="stockHint">현재 저장된 종목 정보와 기록의 출력용 요약입니다. 공부노트의 서식·관계도, 세부 확인 변경 이력은 원본에서 확인하세요. 완료 여부는 예상의 적중 여부와 별개입니다.</p>';
  panel.style.setProperty('--print-cols','2');
  panel.querySelector('#stockPrintColumns').onchange=function(){panel.style.setProperty('--print-cols',this.value);};
  panel.querySelectorAll('article').forEach(function(article){if(article.textContent.length>1000||article.querySelectorAll('figure').length>2)article.classList.add('printLong');});
