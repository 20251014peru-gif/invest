...
const yahoo={kospi:'^KS11',kosdaq:'^KQ11',usdkrw:'KRW=X',sp500:'^GSPC',nasdaq:'^IXIC',sox:'^SOX',vix:'^VIX',dxy:'DX-Y.NYB',hsi:'^HSI',wti:'CL=F',copper:'HG=F',gold:'GC=F',bdry:'BDRY'};
...
export function chartTitle(r){const c=chartSource(r.id);return c?.kind==='chart'?chartLink(r,r.name+' ↗'):`<a href="${hashRoute('indicators',r.id)}">${esc(r.name)}</a>`;}
